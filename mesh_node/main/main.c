...
void mesh_tr_init(bool force_root, const mesh_cfg_simple_t* cfg, mesh_rx_cb_t on_rx) {
    ...
    // IDF 5.x: mesh_id field uses .addr
    memcpy(mcfg.mesh_id.addr, s_cfg.mesh_id, 6);
    mcfg.channel = s_cfg.channel;

    // Keep router blank; we run an internal, autonomous mesh
    ...
    // NOTE: ap.* fields were removed from mesh_cfg_t in newer IDF; do not touch them here.
...
bool mesh_tr_send_broadcast(const void* data, uint16_t len) {
    mesh_addr_t any;
    for (int i = 0; i < 6; ++i) any.addr[i] = 0xFF;
