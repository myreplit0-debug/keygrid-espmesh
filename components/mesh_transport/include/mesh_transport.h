...
#include <stdint.h>
#include <stdbool.h>
#include "esp_mesh.h"   // gives us mesh_addr_t and esp_mesh_* APIs

#ifdef __cplusplus
extern "C" {
#endif

// RX callback signature: called when a packet is received
typedef void (*mesh_rx_cb_t)(const mesh_addr_t* from,
                             const uint8_t* data,
                             uint16_t len);

// Minimal mesh config struct (shared by root/node)
typedef struct {
    uint8_t  mesh_id[6];       // same across all nodes + root
    uint8_t  channel;          // Wi-Fi channel (e.g. 6)
    char     ap_pass[16];      // password for children AP (8..15 chars)
