...
static void wifi_init(void) {
    ...
    // Create default netifs for mesh (internal helper provided by IDF)
    ...
    // Mesh links are usually happier indoors with PS disabled
    ESP_ERROR_CHECK(esp_wifi_set_ps(WIFI_PS_NONE));
}

/* ---------------- RX task: blocking recv loop ---------------- */
...
static void rx_task(void* arg) {
    ...
    while (1) {
        ...
        // else: loop and wait again
    }
}

/* ---------------- Mesh event handler (logs + root addr) ---------------- */
...
static void mesh_event_handler(void* arg, esp_event_base_t base,
    ...
    default:
        // many events are optional to log
        break;
    }
}
...
void mesh_tr_init(bool force_root, const mesh_cfg_simple_t* cfg, mesh_rx_cb_t on_rx) {
    s_rx_cb = on_rx;
    if (cfg) memcpy(&s_cfg, cfg, sizeof(s_cfg));
    ...
    // Mesh ID (6 bytes) + fixed channel
    memcpy(mcfg.mesh_id.id, s_cfg.mesh_id, 6);
    mcfg.channel = s_cfg.channel;

    // Internal mesh (no upstream router/backhaul for now)
...
bool mesh_tr_get_root_addr(mesh_addr_t* out) {
    ...
    if (mesh_tr_is_root()) {
        // Root's own STA MAC is used as address
        esp_wifi_get_mac(WIFI_IF_STA, out->addr);
        return true;
    }
    // If we already learned root addr via event, return it
...
bool mesh_tr_send_broadcast(const void* data, uint16_t len) {
    mesh_addr_t any;
    for (int i = 0; i < 6; ++i) any.addr[i] = 0xFF;   // FF:FF:FF:FF:FF:FF
